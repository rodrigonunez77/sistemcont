			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<div class="col-md-10">
							<input type="text" class="form-control" id="DESCRIPCION" name="DESCRIPCION" placeholder="Nombre Dependencia" data-validation="required">
						</div>
					</div>
...
<script>
	...
	$('#dataUpdate').on('show.bs.modal', function (event) {
		...
		//alert(idSucursal);
		var modal = $(this);
		modal.find('.modal-title').text('Modificar Dependencia');
		modal.find('.modal-body #ID').val(idDependencia);
		modal.find('.modal-body #DEPARTAMENTO').val(departamento);
		modal.find('.modal-body #TIPO').val(tipo);
		modal.find('.modal-body #DESCRIPCION').val(descripcion);
		modal.find('.modal-body #DIRECCION').val(direccion);
		modal.find('.modal-body #SISTEMAS').val(sistemas);
