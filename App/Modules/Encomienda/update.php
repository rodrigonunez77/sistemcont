...
*/
    if($_POST['LIQUIDOPAGABLE']<=0){
	    ...
	    $liquidacion->setCodigoUnico($codigoUnico.'-liquidacion-update');
	    LiquidacionesTransaction::update($liquidacion);
		
		Table::executeQuery("Delete from detalledescuento where IDLIQUIDACION=".$_POST['IDLIQUIDACION']);
		Table::executeQuery("Delete from detalledestino where IDLIQUIDACION=".$_POST['IDLIQUIDACION']);
	    ///detalle descuento
	    //$aporteOfi=0;
	    //$retencion=0;
	    for ($i=1; $i <=15 ; $i++) { 
	    	# code...
	   		if (isset($_POST['td-descripcion-'.$i]) && trim($_POST['td-descripcion-'.$i])!='') {
			    ...
			    $detalleDescuento->setIdLiquidacion($_POST['IDLIQUIDACION']);
	    ...
	    for ($i=1; $i <=15 ; $i++) { 
	    	...
	    	if (isset($_POST['td-localidad-'.$i]) && trim($_POST['td-localidad-'.$i])!='') {
			    ...
			    $detalleDestino->setIdLiquidacion($_POST['IDLIQUIDACION']);
	    ...
	    $log->setCodigoUnico($codigoUnico.'-liquidacion-update');
