	...
	*/
	...
	switch ($_POST['CONDICION']) 
    {
		...
		case 'BUSCANUMERACION':
			...
			$Data=Table::getListQuery("Select MAX(NRORECIBOINGRESO) from movimientos where TIPOREGISTRO='".$_POST['TIPOREGISTRO']."' AND TIPOCONCEPTO!='GENERAL' ");
			if(count($Data)>0){
				$numero=str_pad(($Data[0][0]+1),8,"0",STR_PAD_LEFT);
			}
			else{
